from sqlmodel import select
from sqlmodel.ext.asyncio.session import AsyncSession

from app.domain.entity.transactions import Transaction
from app.infrastructure.database.repositories.base import BaseSQLRepository


class TransactionRepository(BaseSQLRepository[Transaction, Transaction]):
    def __init__(self, session: AsyncSession):
        super().__init__(session=session, model_cls=Transaction, entity_cls=Transaction)

    async def get_by_invoice_number(self, invoice_number: str) -> Transaction | None:
        statement = select(self.model_cls).where(self.model_cls.invoice_number == invoice_number)
        result = await self.session.exec(statement)
        return self.to_domain(result.first())